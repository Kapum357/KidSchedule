 ...
 * Complexity: O(1)
 */
function engineGenerateRefreshToken(): string {
  const bytes = new Uint8Array(32);
  crypto.getRandomValues(bytes);
 ...
 */
function engineGenerateAccessToken(
  userId: string,
  email: string,
  sessionId: string,
  expiresAt: Date
...
export class AuthEngine {
  ...
  const mockUsers: MockUserRecord[] = [
    ...
    const existingUser = mockUsers.find((u) => u.email.toLowerCase() === email.toLowerCase()) ?? null;
    if (existingUser) {
      return {
        success: false,
        error: "invalid_credentials",
  ...
  issueSession(
    ...
    const accessToken = engineGenerateAccessToken(userId, email, sessionId, accessExpiresAt);
    const refreshToken = engineGenerateRefreshToken();
  ...
  initiatePasswordReset(
    ...
    const rawToken = engineGenerateRefreshToken(); // Same entropy as refresh tokens
    const now = new Date();
