/**
 * KidSchedule – Counter Change Request API Route
 *
 * POST /api/calendar/change-requests/[id]/counter
 */
...
export async function POST(
  ...
  const route = "/api/calendar/change-requests/[id]/counter";
  ...
  try {
    ...
    if (request.requestedBy === parent.id) {
      observeApiRequest({ route, method: "POST", status: 403, durationMs: Date.now() - startedAt });
      return NextResponse.json({ error: "Cannot counter your own request" }, { status: 403 });
    }
    ...
    if (!responseNote || responseNote.length < 5) {
      observeApiRequest({ route, method: "POST", status: 400, durationMs: Date.now() - startedAt });
      return NextResponse.json(
        { error: "Counter-proposal note is required (min 5 characters)" },
        { status: 400 }
      );
    }

    await db.scheduleChangeRequests.counter(id, parent.id, responseNote);

    logEvent("info", "change_request.countered", {
      requestId: id,
      familyId: parent.familyId,
      counteredBy: parent.id,
    });

    observeApiRequest({ route, method: "POST", status: 204, durationMs: Date.now() - startedAt });
    return new NextResponse(null, { status: 204 });
  } catch (error) {
    logEvent("error", "POST /api/calendar/change-requests/[id]/counter error", {
      error: error instanceof Error ? error.message : "unknown",
    });
    observeApiRequest({ route, method: "POST", status: 500, durationMs: Date.now() - startedAt });
    console.error("[POST /api/calendar/change-requests/[id]/counter]", error);
